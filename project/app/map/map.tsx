...
async function loadMapNodes(graph: Graph, paperID: string, depth: number) {
  ...
  for (const node of nodes) {
    ...
    try {
      if (firstSet) {
        graph.addNode(node.id, { x: getRandomNumber(0, 25), y: getRandomNumber(0, 25), size: mapToRange(15, 30, citations, nodes.length), label: truncateTitle(node.title), color: blendColors(node.unis) });
      } else {
